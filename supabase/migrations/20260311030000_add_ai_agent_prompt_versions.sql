CREATE TABLE IF NOT EXISTS public.ai_agent_prompt_versions (
  ...
  agent_key TEXT NOT NULL CHECK (agent_key IN ('diet_generation', 'training_generation', 'training_periodization')),
  agent_label TEXT NOT NULL,
  prompt_format TEXT NOT NULL CHECK (prompt_format IN ('text', 'markdown', 'xml', 'json')),
...
CREATE INDEX IF NOT EXISTS idx_ai_agent_prompt_versions_agent_key_created_at
  ON public.ai_agent_prompt_versions(agent_key, created_at DESC);

CREATE UNIQUE INDEX IF NOT EXISTS idx_ai_agent_prompt_versions_active_per_agent
  ON public.ai_agent_prompt_versions(agent_key)
  WHERE is_active = true;

CREATE OR REPLACE FUNCTION public.update_ai_agent_prompt_versions_updated_at()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
...
DROP TRIGGER IF EXISTS update_ai_agent_prompt_versions_updated_at
  ON public.ai_agent_prompt_versions;

CREATE TRIGGER update_ai_agent_prompt_versions_updated_at
  BEFORE UPDATE ON public.ai_agent_prompt_versions
  FOR EACH ROW
  EXECUTE FUNCTION public.update_ai_agent_prompt_versions_updated_at();

ALTER TABLE public.ai_agent_prompt_versions ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Admins can view ai agent prompt versions" ON public.ai_agent_prompt_versions;
CREATE POLICY "Admins can view ai agent prompt versions"
  ON public.ai_agent_prompt_versions
  FOR SELECT
  TO authenticated
  USING (
...
DROP POLICY IF EXISTS "Admins can insert ai agent prompt versions" ON public.ai_agent_prompt_versions;
CREATE POLICY "Admins can insert ai agent prompt versions"
  ON public.ai_agent_prompt_versions
  FOR INSERT
  TO authenticated
  WITH CHECK (
...
DROP POLICY IF EXISTS "Admins can update ai agent prompt versions" ON public.ai_agent_prompt_versions;
CREATE POLICY "Admins can update ai agent prompt versions"
  ON public.ai_agent_prompt_versions
  FOR UPDATE
  TO authenticated
  USING (
...
WHERE NOT EXISTS (
  SELECT 1
  FROM public.ai_agent_prompt_versions
  WHERE agent_key = 'diet_generation'
);
...
WHERE NOT EXISTS (
  SELECT 1
  FROM public.ai_agent_prompt_versions
  WHERE agent_key = 'training_periodization'
);
