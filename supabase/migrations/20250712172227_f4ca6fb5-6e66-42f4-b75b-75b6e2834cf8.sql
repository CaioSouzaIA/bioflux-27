-- Função para resetar formulários automaticamente a cada 30 dias
...
AS $$
BEGIN
  -- Resetar formulários para assinaturas ativas onde passaram 30 dias desde o último reset
  ...
  AND (
    ...
    OR last_reset_date < (now() - INTERVAL '30 days')
  );
...
BEGIN
  ...
  IF NEW.status = 'ativo' AND (
    NEW.last_reset_date IS NULL 
    OR NEW.last_reset_date < (now() - INTERVAL '30 days')
  ) THEN
...
-- Aplicar o trigger na tabela de assinaturas
DROP TRIGGER IF EXISTS trigger_auto_reset_forms ON public.client_subscriptions;
CREATE TRIGGER trigger_auto_reset_forms
  BEFORE UPDATE ON public.client_subscriptions
  FOR EACH ROW EXECUTE FUNCTION check_and_reset_forms();